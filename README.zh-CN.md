...
| 合盖接显示器 / 合盖收纳 | 不让睡的话内屏一直亮着 —— macOS 不因合盖关背光，纯白耗电 |
...
| **LidKeep** | ✅ | ✅ | ✅ | ✅ | **热键零授权**（合盖装一次助手） |

差别只有一处：**LidKeep 灭的是背光，不是显示器电源**。显示器从不睡眠，帧缓冲一直在渲染，所以远程端拿到的总是真实画面，不是一片黑。
...
**电源方案**沿用 Windows「电源选项」思路：插电和用电池是两套设置，拔充电器几秒内自动切换 —— 标题里的**电源 / 电池**就是此刻生效的那套。
...
三项互不排斥：前两个分别管系统睡眠和显示器睡眠，合盖行为独立。**息屏后保持唤醒**打开那一刻就自己持有防睡断言，所以不点「关闭显示器」、让屏幕自己熄掉也照样有效。方案若暂时起不来（缺助手、电量低于下限），标题写明「未生效」，条件满足自动补起，你的设置不会被偷偷改掉。

## 三种常见用法

- **Mac 当远程主机**（UURemote / ToDesk / VNC / SSH）—— `lidkeep off` 熄屏，机器不睡，远程画面正常；回来按热键恢复。怕忘？默认 12 小时兜底自动恢复。
...
Release 产物用 **ad-hoc 临时签名，没走公证**（本项目没有付费 Apple 开发者证书），Gatekeeper 把下载副本当不可信。macOS 26 实测：

| 途径 | Gatekeeper 行为 |
|---|---|
| 直接下载 `.pkg` / `.dmg` / zip | `spctl` 对安装包**和** App 均判 `rejected` |
...
**正解是公证** —— Developer ID 签名加公证票据。做完后上述路径都变普通双击，这是下个版本最高优先级。在此之前，每个 Release 都附带 `SHA256SUMS` 与构建来源证明（SLSA），可自行核验下载物确实出自本仓库：
...
`github.com` 的 Release 二进制资产在国内经常完全不可达 —— 实测**10 秒下载 0 字节**，而 `api.github.com`、`raw.githubusercontent.com` 正常。这是 CDN 层阻断，与本项目无关。
...
**合盖不睡眠**：菜单栏点开**合盖时**选**保持唤醒**即可，无需终端。合盖后内屏熄灭、机器持续运行，下载 / 远程 / 外接显示 / 长任务照常。守护经 SMC 检测合盖（MSLD 键），合盖时内屏亮度归零、开盖自动恢复；只动内屏，外接不受影响；守护停止（电量下限 / 超时 / 手动关）时也恢复，不留黑屏残局。可在**通用**页用「合盖时熄灭内屏」单独关掉。标志写入配置，App / 电脑重启后自动恢复守护。
...
- **持有者记账**：`disablesleep` 是唯一的全局开关，「关屏联动」和「手动防睡」可能同时依赖它。助手记每个持有者，任一方停止只注销自己 —— 不会顺手关掉别人正在用的防睡（账本 `/var/db/lidkeep-nosleep`，root 拥有，普通用户伪造不了）
- **与远控共存**：ToDesk / Sunlogin / UURemote / TeamViewer 等远控用同一开关保持在线。检测到它们在跑，`doctor` 说明「由远控软件持有」，不误报成待修残留
- 电量下限对防睡同样生效 —— 合盖 + 电池 + 不睡是最容易耗干电池的组合

## 原理

不用 macOS 的「显示器睡眠」（那会停掉帧缓冲，远程看不到画面），而是把**系统亮度归零**，同时用 `caffeinate -di` 持有断言，保证显示管线满功率。实测：
...
代价是刻意的：真·显示器睡眠能多省约 0.5–1.5 W，但远程画面就没了。LidKeep 保证机器始终可被远程操控。
...
- **零授权热键**。全局热键走 Carbon `RegisterEventHotKey`，由 WindowServer 直接派发 —— 不需要「辅助功能」「输入监控」任何授权，反复重编译重装也不失效（ad-hoc 签名二进制每次重编译都会丢 TCC 授权，是同类小工具最常见的坑）。
...
**为什么不用 `pmset displaysleepnow`？** 真·显示器睡眠会拆掉帧缓冲，远程端啥都看不到；而且很多 App（浏览器、Electron）持有 `NoDisplaySleepAssertion`，根本进不了显示器睡眠。亮度归零在任何情况下都有效，且是唯一保持远程画面可用的方法。
...
- **部分外接显示器关不掉。** 亮度归零依赖软件亮度接口，多数 HDMI / DVI / DP 外接屏不支持，关屏时不会熄灭（`lidkeep doctor` 具体指出哪一块）。要让它们也灭只能硬件睡眠，那会中断远程画面 —— 本工具刻意不做。
- 屏幕**并非断电** —— 这是刻意设计。背光设为 0，帧缓冲仍在渲染，屏幕共享 / 远程桌面仍能取帧。真·显示器休眠会中断远程访问，详见[原理](#原理)。
- **关屏不等于锁屏。** 关屏期间任何能碰到键盘鼠标的人仍可操作机器，只是看不见画面。离座前请手动锁屏（⌃⌘Q）。
